...
// Process update before sending HTML output
...
// Fetch supplier data
$row = []; // Initialize to avoid undefined errors
if (isset($_GET['id'])) {
    $id = $_GET['id'];
            ...
            <form action="<?= $_SERVER['PHP_SELF'] ?>" method="post">
                <div class="column">
                    ...
                    <p>
                        ...
                        <input type="text" name="sname" value="<?= $row[1] ?? ''; ?>">
                    </p>
                    <p>
                        <label for="sadd">Address:</label><br>
                        <input type="text" name="sadd" value="<?= $row[2] ?? ''; ?>">
                    </p>
                </div>
                <div class="column">
                    <p>
                        <label for="sphno">Phone Number:</label><br>
                        <input type="number" name="sphno" value="<?= $row[3] ?? ''; ?>">
                    </p>
                    <p>
                        <label for="smail">Email Address </label><br>
                        <input type="text" name="smail" value="<?= $row[4] ?? ''; ?>">
                    </p>
                </div>
